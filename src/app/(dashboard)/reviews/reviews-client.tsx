...
import type { Conversation, ConversationMessage, ConversationState, ReviewDecision, SetterDraft } from "@/domain/conversations/types";
 ...
 * Review actions mirror `applyReviewDecision`'s shape locally but do not
 * persist yet (no write path wired for this phase).
 */
...
export function ReviewsClient({
  ...
  const accountsById = useMemo(() => new Map(accountBundles.map((bundle) => [bundle.account.id, bundle.account])), [accountBundles]);
  const campaignsById = useMemo(() => new Map(campaigns.map((campaign) => [campaign.id, campaign])), [campaigns]);
  const draftsByMessageId = useMemo(() => new Map(setterDrafts.map((draft) => [draft.conversationMessageId, draft])), [setterDrafts]);
  ...
  const goToNext = useCallback(() => {
    ...
    const next = conversations[(index + 1) % conversations.length];
