...
function campaignMetrics(campaign: Campaign) {
  ...
  const sent = seedOutreachQueueItems.filter((q) => q.campaignId === campaign.id).length;
  const replyRate = sent > 0 ? Math.round((conversations.length / sent) * 100) : 0;
  const engine = seedEngineTargets.find((e) => e.engineType === campaign.engineType);
    ...
    <Tabs items={DETAIL_TABS} defaultValue="overview">
      {(active) => {
        ...
        if (active === "outreach") {
          return (
            <p className="text-sm text-text-muted">
              {seedOutreachQueueItems.filter((q) => q.campaignId === campaign.id).length} queue items scheduled
        ...
        if (active === "setter") {
          return (
            <p className="text-sm text-text-muted">
              Offer: <span className="font-medium text-text">{seedOffer.name}</span> · {metrics.replies} conversation
