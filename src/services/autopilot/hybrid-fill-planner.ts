...
export function planHybridFill(input: HybridFillPlanInput): HybridFillPlan {
  ...
  if (input.seedInventoryExhausted) return { active: false, rawCount: 0, targetRisk: "target_at_risk_exhaustion", seed: null, reason: "approved Maps Fast seed inventory is exhausted." };
  ...
  return {
    ...
    reason: `hybrid_fill: late-day deficit rescue using approved Maps Fast seed ${seed.query} / ${seed.geography}.`,
  };
}